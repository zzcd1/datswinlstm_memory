...
class SEVIRSkillScore(Metric):
    ...
    def update(self, pred: torch.Tensor, target: torch.Tensor):
        ...
        self.hits = [0] * len(self.threshold_list)
        self.misses = [0] * len(self.threshold_list)
        self.fas = [0] * len(self.threshold_list)
    ...
    @staticmethod
    def calc_seq_hits_misses_fas_static(target, pred, threshold, reduce_dims):
        with torch.no_grad():
            t, p = _threshold(target, pred, threshold)
            hits = torch.sum(t * p, dim=reduce_dims).int()
            misses = torch.sum(t * (1 - p), dim=reduce_dims).int()
            fas = torch.sum((1 - t) * p, dim=reduce_dims).int()
        return hits, misses, fas
